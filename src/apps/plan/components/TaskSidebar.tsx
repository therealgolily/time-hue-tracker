...
const TaskSidebar = ({ tasks, groups, onUpdateTask, onDeleteTask, onNewTask, onClose }: Props) => {
  ...
  return (
    <div
      ...
      style={{ width: 280, fontFamily: "'Caveat', cursive" }}
    >
      <div className="flex items-center justify-between px-4 py-3 border-b-2 border-foreground flex-shrink-0">
        <span style={{ fontSize: 17, fontWeight: 700 }}>Tasks ({tasks.length})</span>
      ...
      <div className="flex-1 overflow-y-auto">
        {tasks.length === 0 && (
          <div className="p-6 text-center opacity-50" style={{ fontSize: 14 }}>No tasks yet.<br />Add one above.</div>
        )}
        {tasks.map(task => {
          ...
          return (
            <div key={task.id} className="border-b border-foreground/20">
              ...
              >
                <div style={{ width: 10, height: 10, background: sc.bar, border: `1.5px solid ${sc.border}`, flexShrink: 0 }} />
              ...
              {isOpen && (
                <div className="px-3 pb-3 flex flex-col gap-2">
                  ...
                  <select
                    ...
                    style={{ fontSize: 13, fontFamily: "'Caveat', cursive", border: '1.5px solid currentColor', padding: '2px 6px', background: sc.bar, color: sc.text, width: '100%' }}
                  >
                  ...
                  <button
                    ...
                    style={{ fontSize: 12, textAlign: 'left', opacity: 0.5, fontFamily: "'Caveat', cursive" }}
      ...
      <div className="border-t-2 border-foreground px-3 py-3 flex-shrink-0">
        <div style={{ fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 6, opacity: 0.5 }}>Legend</div>
        <div className="flex flex-col gap-1">
          {(Object.keys(STATUS_LABELS) as TaskStatus[]).map(s => (
            <div key={s} className="flex items-center gap-2">
              <div style={{ width: 14, height: 10, background: STATUS_COLORS[s].bar, border: `1.5px solid ${STATUS_COLORS[s].border}`, filter: 'url(#sketchy)' }} />
